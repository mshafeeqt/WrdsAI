...
def _is_class_11_physics_record(record: dict) -> bool:
    return _record_pdf(record).lower().startswith("class 11/physics/")


def _restore_marker_setting(previous_value: str | None) -> None:
    if previous_value is None:
        os.environ.pop("EXACT_USE_MARKER", None)
    else:
        os.environ["EXACT_USE_MARKER"] = previous_value


def index_class_11_physics_exact() -> None:
    """Build exact retrieval JSON and figure metadata for Class 11 Physics PDFs only."""
    logging.basicConfig(level=logging.INFO, format="%(levelname)s:%(name)s:%(message)s")
    ensure_exact_dirs()

    root = settings.math_data_dir
    target_dir = root / "Class 11" / "Physics"
    ...
    os.environ["EXACT_USE_MARKER"] = "false"
    ...
    print(f"Found {len(pdf_files)} Class 11 Physics PDFs")
    try:
        for pdf_path in tqdm(pdf_files, desc="Class 11 Physics exact", unit="pdf"):
        ...
        _restore_marker_setting(previous_marker_setting)

    existing_pages = read_json(PAGE_INDEX_DIR / "all_pages.json", [])
    existing_questions = read_json(QUESTION_INDEX_DIR / "all_questions.json", [])

    merged_pages = [record for record in existing_pages if not _is_class_11_physics_record(record)]
    merged_questions = [record for record in existing_questions if not _is_class_11_physics_record(record)]
    ...
    print(
        "Class 11 Physics exact indexing complete: "
